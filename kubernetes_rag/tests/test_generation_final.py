"""Final comprehensive test suite for generation module to achieve 100% coverage."""
...
class TestCreateRAGGenerator:
    ...
    def test_create_rag_generator_with_custom_config(self):
        ...
        custom_config.llm.model_name = "claude-3-opus"
...
class TestGenerationIntegration:
    ...
    def test_generation_with_custom_models(self):
        ...
        with patch.dict(os.environ, {"TESTING": "true"}):
            ...
            llm = create_llm(provider="anthropic", model="claude-3-opus")
...
class TestRAGGeneratorConversational:
    """Test RAGGenerator conversational features."""

    def test_generate_conversational_answer(self):
        """Test RAGGenerator generate_conversational_answer method."""
        with patch.dict(os.environ, {"TESTING": "true"}):
            llm = OpenAILLM()
            generator = RAGGenerator(llm=llm)
            ...
            result = generator.generate_conversational_answer(
                "What is Kubernetes?", 
                documents, 
                history
            )
    ...
    def test_generate_conversational_answer_empty_history(self):
        """Test RAGGenerator generate_conversational_answer with empty history."""
        with patch.dict(os.environ, {"TESTING": "true"}):
            llm = OpenAILLM()
            generator = RAGGenerator(llm=llm)
    ...
    def test_generate_conversational_answer_none_history(self):
        """Test RAGGenerator generate_conversational_answer with None history."""
        with patch.dict(os.environ, {"TESTING": "true"}):
            llm = OpenAILLM()
            generator = RAGGenerator(llm=llm)
    ...
    def test_build_context(self):
        ...
        with patch.dict(os.environ, {"TESTING": "true"}):
            ...
            documents = [
                {"content": "Document 1", "metadata": {"source": "doc1.md"}},
                {"content": "Document 2", "metadata": {"source": "doc2.md"}}
            ]
    ...
    def test_create_conversational_prompt(self):
        """Test RAGGenerator _create_conversational_prompt method."""
        with patch.dict(os.environ, {"TESTING": "true"}):
            llm = OpenAILLM()
            generator = RAGGenerator(llm=llm)
            ...
            prompt = generator._create_conversational_prompt(query, context, history)
