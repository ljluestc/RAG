...
(function() {
        ...
        class autoAcceptAndAnalytics {
            constructor(interval = 2000) {
                ...
                this.config = {
                    ...
                    enableCancel: true,            // New: Cancel button to prevent stuck jobs
                    enableStop: true,              // New: Stop button for running processes
                    enableAbort: true              // New: Abort button for operations
                };
            ...
            calculateTimeSaved(buttonType) {
                ...
                const workflowTimeSavings = {
                    ...
                    'cancel': this.roiTracking.averageCompleteWorkflow + 1000, // time saved by preventing stuck jobs
                    'stop': this.roiTracking.averageCompleteWorkflow + 1500, // time saved by stopping processes
                    'abort': this.roiTracking.averageCompleteWorkflow + 2000 // time saved by aborting operations
                };
            ...
            normalizeButtonType(buttonType) {
                ...
                if (type.includes('abort')) return 'abort';
            ...
            createControlPanel() {
                ...
                const configOptions = [
                    ...
                    { id: 'aa-cancel', text: 'Cancel', checked: true },
                    { id: 'aa-stop', text: 'Stop', checked: true },
                    { id: 'aa-abort', text: 'Abort', checked: true }
                ];
            ...
            renderAnalyticsTab(container) {
                ...
                if (this.analytics.buttonTypeCounts && Object.keys(this.analytics.buttonTypeCounts).length > 0) {
                    ...
                    Object.entries(this.analytics.buttonTypeCounts).forEach(([type, count]) => {
                        ...
                        } else if (type === 'cancel' || type === 'stop' || type === 'abort') {
                            valueSpan.style.color = '#F44336'; // Red for cancel/stop/abort actions
                        } else {
                ...
                checkboxes.forEach(checkbox => {
                    checkbox.addEventListener('change', () => {
                        const configMap = {
                            ...
                            'aa-cancel': 'enableCancel',
                            'aa-stop': 'enableStop',
                            'aa-abort': 'enableAbort'
                        };
            ...
            isAcceptButton(element) {
                ...
                const patterns = [
                    ...
                    { pattern: 'cancel', enabled: this.config.enableCancel },
                    { pattern: 'stop', enabled: this.config.enableStop },
                    { pattern: 'abort', enabled: this.config.enableAbort }
                ];
            ...
            clickElement(element) {
                try {
                    ...
                    if (isResumeLink) {
                        ...
                        const timeSaved = this.calculateTimeSaved('cancel');
                        this.logToPanel(`❌ Cancel clicked [saved ${this.formatTimeDuration(timeSaved)}]`, 'warning');
                        this.log(`Cancel clicked - Time saved: ${this.formatTimeDuration(timeSaved)}`);

                        // Track button type count
                        if (!this.analytics.buttonTypeCounts) {
                            this.analytics.buttonTypeCounts = {};
                        }
                        this.analytics.buttonTypeCounts['cancel'] = (this.analytics.buttonTypeCounts['cancel'] || 0) + 1;
                        ...
                        // Handle Stop button
                        const timeSaved = this.calculateTimeSaved('stop');
                        this.logToPanel(`⏹️ Stop clicked [saved ${this.formatTimeDuration(timeSaved)}]`, 'warning');
                        this.log(`Stop clicked - Time saved: ${this.formatTimeDuration(timeSaved)}`);

                        // Track button type count
                        if (!this.analytics.buttonTypeCounts) {
                            this.analytics.buttonTypeCounts = {};
                        }
                        this.analytics.buttonTypeCounts['stop'] = (this.analytics.buttonTypeCounts['stop'] || 0) + 1;
                        ...
                        // Handle Abort button
                        const timeSaved = this.calculateTimeSaved('abort');
                        this.logToPanel(`🛑 Abort clicked [saved ${this.formatTimeDuration(timeSaved)}]`, 'warning');
                        this.log(`Abort clicked - Time saved: ${this.formatTimeDuration(timeSaved)}`);

                        // Track button type count
                        if (!this.analytics.buttonTypeCounts) {
                            this.analytics.buttonTypeCounts = {};
                        }
                        this.analytics.buttonTypeCounts['abort'] = (this.analytics.buttonTypeCounts['abort'] || 0) + 1;
            ...
            isWindsurfAcceptButton(element) {
                ...
                // Additional check for reject buttons - exclude them (but allow cancel/stop/abort)
                if (text.includes('reject') && !text.includes('cancel') && !text.includes('stop') && !text.includes('abort')) {
                    if (this.debugMode) {
    ...
    if (!globalThis.simpleAccept) {
        ...
        console.log('Types: "acceptAll", "accept", "run", "runCommand", "apply", "execute", "resume", "connectionResume", "tryAgain", "cancel", "stop", "abort"');
    }
})();
